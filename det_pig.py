...
PATH_TO_LABELS = os.path.join('./object_detection/data', 'oid_bbox_trainable_label_map.pbtxt')  # 'mscoco_label_map.pbtxt')
...
if test_mode:
	...
	PATH_TO_TEST_IMAGES_DIR = './pig_data/train_folder/'
	TARGET_FOLDER = './pig_data/train_folder_det/'
...
with detection_graph.as_default():
	with tf.Session(graph=detection_graph) as sess:
		...
		if test_mode:
			...
			TEST_IMAGE = os.listdir('./pig_data/train_folder/')
			TEST_IMAGE_PATHS = [item for item in TEST_IMAGE]
            # print TEST_IMAGE_PATHS
			image = cv2.imread(os.path.join('./pig_data/train_folder/', TEST_IMAGE_PATHS[0]))
			...
			for j in range(0, len(TEST_IMAGE_PATHS), BATCH_SIZE):
				...
				for k in range(0, BATCH_SIZE):
					image = cv2.imread(os.path.join('./pig_data/train_folder/', TEST_IMAGE_PATHS[j + k]))
